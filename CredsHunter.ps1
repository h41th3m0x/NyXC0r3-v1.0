<#
.SYNOPSIS
    Retrieves saved credentials from browsers and Windows credential manager
.DESCRIPTION
    This script attempts to extract saved credentials from:
    - Windows Credential Manager
    - Chrome browser
    - Edge browser
    - Firefox browser (if master password is known)
.NOTES
    Requires administrative privileges for some operations
    Firefox requires master password if one is set
#>
...
function Get-WindowsCredentials {
    ...
    Write-Host "`n=== Windows Credential Manager ===`n" -ForegroundColor Yellow
    
    try {
        # Get generic credentials
        $creds = cmdkey /list | Where-Object { $_ -match "Target:" }
        
        if (-not $creds) {
            Write-Host "No saved credentials found in Windows Credential Manager" -ForegroundColor Cyan
            return
        }

        foreach ($cred in $creds) {
            $target = $cred -replace "Target:",""
            Write-Host "Found credential entry for: $target" -ForegroundColor Green
            
            # Try to retrieve the actual credential (may not work for all types)
            try {
                $result = cmdkey /list:$target 2>&1
                $result | Where-Object { $_ -match "User" -or $_ -match "Content" } | ForEach-Object {
                    Write-Host "  $_" -ForegroundColor Cyan
                }
            }
            catch {
                Write-Host "  Could not retrieve full details (may require specific decryption)" -ForegroundColor Red
            }
        }
    }
    catch {
        Write-Host "Error accessing Windows Credential Manager: $_" -ForegroundColor Red
    }
}
